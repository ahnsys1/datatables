...
import {
  ...
  StoryDiscussionRealtimeEvent,
  TaskDiscussionMessage,
  TaskDiscussionRealtimeEvent,
  TaskItem,
  TaskStatus,
  TmUser,
  UserStory
...
type FlattenedTaskDiscussionMessage = TaskDiscussionMessage & { depth: number };
...
export class TaskManagementComponent implements OnInit, OnDestroy {
  @ViewChild('taskFormCard')
  private taskFormCardRef?: ElementRef<HTMLDivElement>;

  @ViewChild('sprintStartDateInput')
  set sprintStartDateInputRef(ref: ElementRef<HTMLInputElement> | undefined) {
    this.initializeSprintDatePicker('startDate', ref?.nativeElement, this.sprintStartDatePicker);
  }

  @ViewChild('sprintEndDateInput')
  set sprintEndDateInputRef(ref: ElementRef<HTMLInputElement> | undefined) {
    this.initializeSprintDatePicker('endDate', ref?.nativeElement, this.sprintEndDatePicker);
  }

  activeTopMenu: 'sprints' | 'stories' | 'tasks' | 'my-tasks' = 'sprints';
  ...
  storyEditDialogOpen = false;
  editingStory: UserStory | null = null;
  storyEditSubmitting = false;

  deleteDialog: { story: UserStory } | null = null;
  ...
  taskDiscussionDialogOpen = false;
  taskDiscussionLoading = false;
  taskDiscussionSubmitting = false;
  discussionTask: TaskItem | null = null;
  taskDiscussionMessages: TaskDiscussionMessage[] = [];
  taskDiscussionFlatMessages: FlattenedTaskDiscussionMessage[] = [];
  taskDiscussionDraft = '';
  taskDiscussionReplyTo: FlattenedTaskDiscussionMessage | null = null;
  taskDiscussionErrorMessage = '';
  unreadDiscussionTaskIds = new Set<number>();
  burnoutHover: BurnoutHoverState | null = null;

  private readonly discussionRealtimeClient: Client;
  private discussionRealtimeSubscription?: StompSubscription;
  ...
  constructor(
    ...
    this.discussionRealtimeClient = new Client({
      webSocketFactory: () => new SockJS('/ws'),
      reconnectDelay: 3000,
      heartbeatIncoming: 10000,
      heartbeatOutgoing: 10000,
      debug: () => {
        // keep quiet in normal runtime
      }
    });

    this.discussionRealtimeClient.onConnect = () => {
      this.subscribeToDiscussionRealtime();
      this.subscribeToTaskDiscussionRealtime();
      if (this.currentUser != null && this.selectedProjectId != null) {
        this.refreshUnreadStoryDiscussionIds();
        this.refreshUnreadTaskDiscussionIds();
      }
    };

    this.discussionRealtimeClient.onStompError = frame => {
      console.error('Discussion realtime broker error:', frame.headers['message'], frame.body);
    };
  }

  ngOnInit(): void {
    this.discussionRealtimeClient.activate();
    ...
    this.route.queryParamMap.subscribe(params => {
      ...
      if (requestedView == null) {
        this.activeTopMenu = 'sprints';
      }

      this.closeStoryDiscussion();
      this.closeTaskDiscussion();
  ...
  ngOnDestroy(): void {
    this.unsubscribeFromDiscussionRealtime();
    this.unsubscribeFromTaskDiscussionRealtime();
    if (this.discussionRealtimeClient.active) {
      void this.discussionRealtimeClient.deactivate();
    }
    this.sprintStartDatePicker?.destroy();
    this.sprintEndDatePicker?.destroy();
  }
  ...
  private initializeCurrentUserFromJwt(): void {
    ...
    this.errorMessage = '';
    this.taskService.syncCurrentUserFromJwt()
      .pipe(finalize(() => {
        this.initializingCurrentUser = false;
        this.cdr.markForCheck();
      }))
      .subscribe({
        next: user => {
          ...
          this.taskForm.assigneeUserId = user.id;
          this.taskForm.reviewerUserId = user.id;
          this.users = [];
  ...
  loadUsers(projectId: number | null): void {
    if (projectId == null) {
      this.users = [];
    ...
    this.taskService.getUsers(projectId).subscribe({
      next: data => {
        ...
        const preferredUserId = this.currentUser != null && availableUserIds.has(this.currentUser.id)
          ? this.currentUser.id
          : this.users[0]?.id ?? '';

        if (!availableUserIds.has(this.taskForm.assigneeUserId)) {
          this.taskForm.assigneeUserId = preferredUserId;
        }
        if (!availableUserIds.has(this.taskForm.reviewerUserId)) {
          this.taskForm.reviewerUserId = preferredUserId;
        }
        this.cdr.markForCheck();
      },
      error: err => {
        this.errorMessage = this.resolveApiError(err, 'Nepodařilo se načíst seznam uživatelů projektu.');
        this.cdr.markForCheck();
      }
    });
  }
  ...
  goToProjects(): void {
    this.router.navigate(['/task-management/projects']);
  }
  ...
  createSprint(): void {
    ...
    const parsedStartDate = this.parseSprintInternalDate(startDate);
    const parsedEndDate = this.parseSprintInternalDate(endDate);
    if (parsedStartDate == null || parsedEndDate == null) {
      this.errorMessage = 'Zadejte platná data sprintu.';
      return;
    }
    ...
    this.taskService.createSprint(this.selectedProjectId, name, startDate, endDate)
      .subscribe({
        next: sprint => {
          ...
          this.sprintVisibleDates = { startDate: '', endDate: '' };
          this.syncSprintDatePicker('startDate');
          this.syncSprintDatePicker('endDate');
          this.cdr.markForCheck();
        },
        error: err => {
  ...
  showMenu(menu: 'sprints' | 'stories' | 'tasks' | 'my-tasks'): void {
    ...
    this.taskDisplayMode = 'all';
  }

  handleSprintVisibleDateChange(field: 'startDate' | 'endDate', rawValue: string): void {
    const parsed = this.parseDmy(rawValue);
    if (parsed == null) {
      this.syncSprintDatePicker(field);
      return;
    }

    this.submitSprintDate(field, parsed);
  }
  ...
  onMenuStoryChange(storyId: number, mode: 'all' | 'mine' = 'all'): void {
    ...
    this.activeTopMenu = 'tasks';
    this.taskDisplayMode = mode;
    this.cdr.markForCheck();
  }

  onTaskStoryChange(storyId: number): void {
    const id = Number(storyId);
    if (!id || id <= 0) {
      return;
    }
  ...
  openTaskDiscussion(task: TaskItem, event?: Event): void {
    ...
    this.errorMessage = '';
    this.taskDiscussionDialogOpen = true;
    this.discussionTask = task;
    this.taskDiscussionMessages = [];
    this.taskDiscussionFlatMessages = [];
  ...
  closeTaskDiscussion(): void {
    this.taskDiscussionDialogOpen = false;
    this.taskDiscussionLoading = false;
    this.taskDiscussionSubmitting = false;
  ...
  hasUnreadTaskDiscussion(taskId: number): boolean {
    return this.unreadDiscussionTaskIds.has(taskId);
  }
  ...
  replyToTaskDiscussionMessage(message: FlattenedTaskDiscussionMessage): void {
    this.taskDiscussionReplyTo = message;
    this.cdr.markForCheck();
  }

  cancelTaskDiscussionReply(): void {
    this.taskDiscussionReplyTo = null;
    this.cdr.markForCheck();
  }
  ...
  submitTaskDiscussionMessage(): void {
    if (this.currentUser == null) {
      this.errorMessage = 'Nejdříve se přihlas.';
      return;
    }
    ...
    if (this.discussionTask == null) {
      this.errorMessage = 'Nejdřív vyber task pro diskusi.';
      return;
    }

    const content = this.taskDiscussionDraft.trim();
    if (content === '') {
      this.errorMessage = 'Napiš zprávu do diskuse.';
      return;
    }

    this.taskDiscussionSubmitting = true;
    this.taskService.createTaskDiscussionMessage({
      taskId: this.discussionTask.id,
      authorUserId: this.currentUser.id,
      parentMessageId: this.taskDiscussionReplyTo?.id ?? null,
      content
    }).pipe(finalize(() => {
      this.taskDiscussionSubmitting = false;
      this.cdr.markForCheck();
      ...
      error: err => {
        this.errorMessage = this.resolveApiError(err, 'Odeslání zprávy do diskuse tasku selhalo.');
        this.cdr.markForCheck();
      }
    });
  }
  ...
  confirmDeleteDialog(): void {
    ...
    this.confirmStoryDeletion(this.deleteDialog.story);
  }
  ...
  deleteDialogTitle(): string {
    return 'Potvrzení smazání User Story';
  }

  deleteDialogMessage(): string {
    if (this.deleteDialog != null) {
      return `Opravdu chceš smazat user story "${this.deleteDialog.story.title}" z product backlogu?`;
    }
  ...
  private confirmStoryDeletion(story: UserStory): void {
    this.errorMessage = '';
  ...
  openStoryEditDialog(story: UserStory): void {
    this.errorMessage = '';
    this.editingStory = story;
    this.storyEditForm.title = story.title;
    this.storyEditForm.description = story.description;
    this.storyEditSubmitting = false;
    this.storyEditDialogOpen = true;
    this.cdr.markForCheck();
  }

  closeStoryEditDialog(): void {
    this.storyEditDialogOpen = false;
    this.editingStory = null;
    this.storyEditSubmitting = false;
    this.storyEditForm.title = '';
    this.storyEditForm.description = '';
    this.cdr.markForCheck();
  }

  submitStoryEdit(): void {
    if (this.editingStory == null) {
      return;
    }

    const title = this.storyEditForm.title.trim();
    const description = this.storyEditForm.description.trim();
    ...
    if (description === '') {
      this.errorMessage = 'Popis user story nesmí být prázdný.';
      this.cdr.markForCheck();
      return;
    }

    if (title === this.editingStory.title.trim() && description === this.editingStory.description.trim()) {
      this.closeStoryEditDialog();
      return;
    }

    this.storyEditSubmitting = true;
    this.taskService.updateStory(this.editingStory.id, { title, description }).pipe(
      finalize(() => {
        this.storyEditSubmitting = false;
        this.cdr.markForCheck();
      })
      ...
      error: err => {
        this.errorMessage = this.resolveApiError(err, 'Úprava user story selhala.');
        this.cdr.markForCheck();
      }
    });
  }
  ...
  editTask(task: TaskItem): void {
    ...
    this.taskForm.definitionOfDone = task.definitionOfDone;
    this.taskForm.colorHex = task.colorHex;
    this.taskForm.assigneeUserId = task.assigneeUserId;
    this.taskForm.reviewerUserId = task.reviewerUserId;
    this.cdr.detectChanges();
    this.scrollTaskFormIntoView();
  }
  ...
  submitTaskForm(): void {
    ...
    this.updateTaskFromForm(this.editingTaskId);
  }
  ...
  private updateTaskFromForm(taskId: number): void {
    const title = this.taskForm.title.trim();
    const description = this.taskForm.description.trim();
    const definitionOfDone = this.taskForm.definitionOfDone.trim();
  ...
  updateTaskStatus(task: TaskItem, status: TaskStatus): void {
    ...
    task.status = status; // Optimisticky UI update okamžitě pro rychlou odezvu
    this.cdr.detectChanges();
    ...
    this.taskService.updateTaskStatus(task.id, status, task.reviewComment ?? '')
      .subscribe({
        ...
        error: () => {
          task.status = previousStatus; // Revert upon failure
  ...
  onTaskDragStart(event: DragEvent, task: TaskItem): void {
    if (event.dataTransfer != null) {
      ...
      // Some browsers/framework combinations are more reliable with both mime types.
      event.dataTransfer.setData('text', String(task.id));
    }
    
    // Defer setting the dragging ID to allow the browser to take a drag snapshot
    // of the original element before any CSS classes (like .dragging) are applied.
    // This fixes the "needs to drag twice" (az na podruhe) issue in some browsers.
    setTimeout(() => {
  ...
  onTaskDragEnd(): void {
    ...
    // Clear the dragging task ID so the CSS dragging class is removed.
    // Done in a setTimeout to ensure that the drop event (if any) has 
    // a chance to process before we clear the ID.
    setTimeout(() => {
  ...
  onStatusDragLeave(event: DragEvent): void {
    ...
    // Ignore dragleave if we are just moving into a child element
  ...
  private visibleStoryById(storyId: number): UserStory | undefined {
    return this.storyById(storyId) ?? this.backlogStories.find(story => story.id === storyId);
  }
  ...
  storyTaskCount(storyId: number): number {
    const dashboardHasStory = (this.dashboard?.userStories ?? []).some(story => story.id === storyId);
    if (dashboardHasStory) {
      return (this.dashboard?.tasks ?? []).filter(task => task.storyId === storyId).length;
    }

    return this.visibleStoryById(storyId)?.taskCount ?? 0;
  }
  ...
  storyCompletedTaskCount(storyId: number): number {
    ...
    return this.visibleStoryById(storyId)?.completedTaskCount ?? 0;
  }

  discussionAuthorLabel(message: { authorUserId: string; authorDisplayName: string }): string {
  ...
  sprintCompletedTaskCount(): number {
    return (this.dashboard?.tasks ?? []).filter(task => task.status === 'DONE').length;
  }

  returnedTasks(): TaskItem[] {
    return this.scopedTasks().filter(task => task.status === 'RETURNED');
  }
  ...
  maxRemaining(points: BurnoutPoint[]): number {
    return points.length === 0 ? 1 : Math.max(...points.map(p => p.remainingPoints), 1);
  }
  ...
  burnoutXGrid(points: BurnoutPoint[]): number[] {
    const displayPoints = this.burnoutDisplayPoints(points);
    if (displayPoints.length === 0) {
      return [];
    }
    ...
    return Array.from({ length: displayPoints.length }, (_, index) =>
      this.burnoutPadding.left + (index / denominator) * chartWidth
    );
  }
  ...
  burnoutDropEvents(points: BurnoutPoint[]): Array<{ key: string; x: number; y: number; date: string; dropPoints: number; remaining: number; completedStories: string[]; tooltip: string }> {
    const displayPoints = this.burnoutDisplayPoints(points);
    ...
    const series = this.burnoutSeries(points);
    const events: Array<{ key: string; x: number; y: number; date: string; dropPoints: number; remaining: number; completedStories: string[]; tooltip: string }> = [];

    for (let index = 1; index < displayPoints.length; index++) {
      const dropPoints = Math.max(displayPoints[index - 1].remainingPoints - displayPoints[index].remainingPoints, 0);
      if (dropPoints <= 0) {
        continue;
      }

      events.push({
        key: `${displayPoints[index].date}-${index}`,
        x: series[index].x,
        y: series[index].yRemaining,
        date: displayPoints[index].date,
        dropPoints,
        remaining: displayPoints[index].remainingPoints,
        completedStories: series[index].completedStories,
        tooltip: series[index].tooltip
      });
    }
  ...
  burnoutDropSegments(points: BurnoutPoint[]): Array<{ key: string; x: number; y1: number; y2: number; tooltip: string }> {
    const displayPoints = this.burnoutDisplayPoints(points);
    if (displayPoints.length === 0) {
      return [];
    }
    ...
    const series = this.burnoutSeries(points);
    const totalStoryPoints = this.maxBurnoutValue(points);
    const segments: Array<{ key: string; x: number; y1: number; y2: number; tooltip: string }> = [];
    const firstDayDrop = Math.max(totalStoryPoints - displayPoints[0].remainingPoints, 0);

    if (firstDayDrop > 0 && series[0] != null) {
      segments.push({
        key: `${displayPoints[0].date}-0-drop`,
        x: series[0].x,
        y1: this.burnoutPadding.top,
        y2: series[0].yRemaining,
        tooltip: this.buildBurnoutPointTooltip(
          displayPoints[0].date,
          displayPoints[0].remainingPoints,
          displayPoints[0].completedPoints,
          firstDayDrop,
          series[0].completedStories
        )
      });
    }
    ...
    for (let index = 1; index < displayPoints.length; index++) {
      const dropPoints = Math.max(displayPoints[index - 1].remainingPoints - displayPoints[index].remainingPoints, 0);
      if (dropPoints <= 0 || series[index] == null || series[index - 1] == null) {
        continue;
      }

      segments.push({
        key: `${displayPoints[index].date}-${index}-drop`,
        x: series[index].x,
        y1: series[index - 1].yRemaining,
        y2: series[index].yRemaining,
        tooltip: this.buildBurnoutPointTooltip(
          displayPoints[index].date,
          displayPoints[index].remainingPoints,
          displayPoints[index].completedPoints,
          dropPoints,
          series[index].completedStories
        )
      });
    }
  ...
  private initializeSprintDatePicker(
    field: 'startDate' | 'endDate',
    input: HTMLInputElement | undefined,
    currentPicker: FlatpickrInstance | null
  ): void {
    currentPicker?.destroy();

    if (input == null) {
      if (field === 'startDate') {
        this.sprintStartDatePicker = null;
      } else {
        this.sprintEndDatePicker = null;
      }
      return;
    }
    ...
    const picker = flatpickr(input, {
      locale: Czech,
      dateFormat: 'd.m.Y',
      defaultDate: this.sprintForm[field] === '' ? undefined : this.parseSprintInternalDate(this.sprintForm[field]) ?? undefined,
      allowInput: true,
      disableMobile: true,
      onChange: selectedDates => {
        ...
        this.submitSprintDate(field, selected);
      },
      onClose: (_selectedDates, dateStr) => {
        if (dateStr.trim() === '') {
          this.sprintForm[field] = '';
          this.sprintVisibleDates[field] = '';
          this.syncSprintDatePicker(field);
          return;
        }
    ...
    if (field === 'startDate') {
      this.sprintStartDatePicker = picker;
    } else {
      this.sprintEndDatePicker = picker;
    }

    this.syncSprintDatePicker(field);
  }

  private submitSprintDate(field: 'startDate' | 'endDate', date: Date): void {
    const normalizedDate = new Date(date.getFullYear(), date.getMonth(), date.getDate());
    this.sprintForm[field] = this.formatSprintInternalDate(normalizedDate);
    this.sprintVisibleDates[field] = this.formatDmy(normalizedDate);
    this.syncSprintDatePicker(field);
  }

  private syncSprintDatePicker(field: 'startDate' | 'endDate'): void {
    const picker = field === 'startDate' ? this.sprintStartDatePicker : this.sprintEndDatePicker;
    const value = this.sprintForm[field];
    const parsedValue = value === '' ? null : this.parseSprintInternalDate(value);
    const visibleValue = parsedValue == null ? '' : this.formatDmy(parsedValue);
    this.sprintVisibleDates[field] = visibleValue;
    ...
    picker.setDate(parsedValue, false);
  }

  private parseSprintInternalDate(value: string): Date | null {
  ...
  private formatSprintInternalDate(date: Date): string {
  ...
  burnoutDropTitle(drop: { date: string; dropPoints: number; remaining: number; completedStories: string[] }): string {
    return this.buildBurnoutPointTooltip(drop.date, drop.remaining, 0, drop.dropPoints, drop.completedStories ?? []);
  }

  burnoutCompletedStoriesLabel(completedStories: string[]): string {
    if (completedStories == null || completedStories.length === 0) {
      return 'Bez dokončených stories';
    }

    return `Dokončené stories: ${completedStories.join(', ')}`;
  }
  ...
  private parseIsoDate(value: unknown): Date | null {
    ...
    if (typeof value === 'object') {
      ...
      const month = typeof candidate.monthValue === 'number'
        ? candidate.monthValue
        : (typeof candidate.month === 'number' ? candidate.month : null);
      const day = typeof candidate.dayOfMonth === 'number'
        ? candidate.dayOfMonth
        : (typeof candidate.day === 'number' ? candidate.day : null);
      ...
      const millisecond = typeof candidate.nano === 'number'
        ? Math.floor(candidate.nano / 1_000_000)
        : 0;
  ...
  private resolveApiError(err: unknown, fallback: string): string {
    ...
    if (httpError.status === 403) {
      return 'Do vybraného projektu nemáš oprávnění vstoupit nebo v něm provést tuto akci.';
    }
  ...
  private loadTaskDiscussionMessages(markAsRead: boolean): void {
    if (this.discussionTask == null) {
      return;
    }

    this.taskDiscussionLoading = true;
    this.taskService.getTaskDiscussionMessages(this.discussionTask.id)
      .pipe(finalize(() => {
        this.taskDiscussionLoading = false;
        this.cdr.markForCheck();
      }))
      .subscribe({
        next: messages => {
          this.taskDiscussionErrorMessage = '';
          this.taskDiscussionMessages = messages;
          this.taskDiscussionFlatMessages = this.flattenTaskDiscussionMessages(messages);
          if (markAsRead) {
            this.markOpenedTaskDiscussionAsRead();
          }
          this.cdr.markForCheck();
        },
        error: err => {
          this.taskDiscussionMessages = [];
          this.taskDiscussionFlatMessages = [];
          this.taskDiscussionErrorMessage = this.resolveApiError(err, 'Načtení diskuse tasku selhalo.');
          this.cdr.markForCheck();
        }
      });
  }
  ...
  private flattenTaskDiscussionMessages(
    messages: TaskDiscussionMessage[],
    depth = 0
  ): FlattenedTaskDiscussionMessage[] {
    return messages.flatMap(message => [
      { ...message, depth },
      ...this.flattenTaskDiscussionMessages(message.replies ?? [], depth + 1)
    ]);
  }
  ...
  private markOpenedStoryDiscussionAsRead(): void {
    ...
    this.taskService.markStoryDiscussionRead(discussionStoryId, this.currentUser.id)
      .subscribe({
        next: () => {
          // unread state is synchronized through websocket events
        },
        error: () => {
          // Keep the UI usable even if read-state sync fails.
        }
      });
  }

  private markOpenedTaskDiscussionAsRead(): void {
    if (this.currentUser == null || this.discussionTask == null) {
      return;
    }

    const discussionTaskId = this.discussionTask.id;
    this.taskService.markTaskDiscussionRead(discussionTaskId, this.currentUser.id)
      .subscribe({
        next: () => {
          // unread state is synchronized through websocket events
        },
  ...
  private subscribeToDiscussionRealtime(): void {
    this.unsubscribeFromDiscussionRealtime();

    if (this.currentUser == null || this.selectedProjectId == null || !this.discussionRealtimeClient.connected) {
      return;
    }

    const projectId = this.selectedProjectId;
    this.discussionRealtimeSubscription = this.discussionRealtimeClient.subscribe(
      `/topic/task-management.projects.${projectId}.discussion`,
      (message: IMessage) => {
        try {
          const event = JSON.parse(message.body) as StoryDiscussionRealtimeEvent;
          this.handleDiscussionRealtimeEvent(event);
        } catch {
          this.refreshUnreadStoryDiscussionIds();
        }
      }
  ...
  private subscribeToTaskDiscussionRealtime(): void {
    this.unsubscribeFromTaskDiscussionRealtime();
    ...
    const projectId = this.selectedProjectId;
    this.taskDiscussionRealtimeSubscription = this.discussionRealtimeClient.subscribe(
      `/topic/task-management.projects.${projectId}.task-discussion`,
      (message: IMessage) => {
        try {
          const event = JSON.parse(message.body) as TaskDiscussionRealtimeEvent;
          this.handleTaskDiscussionRealtimeEvent(event);
        } catch {
          this.refreshUnreadTaskDiscussionIds();
        }
      }
  ...
  private unsubscribeFromDiscussionRealtime(): void {
    this.discussionRealtimeSubscription?.unsubscribe();
    this.discussionRealtimeSubscription = undefined;
  }

  private unsubscribeFromTaskDiscussionRealtime(): void {
    this.taskDiscussionRealtimeSubscription?.unsubscribe();
    this.taskDiscussionRealtimeSubscription = undefined;
  }

  private handleDiscussionRealtimeEvent(event: StoryDiscussionRealtimeEvent): void {
    if (this.currentUser == null || this.selectedProjectId == null || event.projectId !== this.selectedProjectId) {
      return;
    }

    if (event.eventType === 'MESSAGE_CREATED') {
      const openedStoryId = this.discussionDialogOpen ? this.discussionStory?.id : null;
      const isOpenedDiscussion = openedStoryId != null && openedStoryId === event.storyId;

      if (event.actorUserId === this.currentUser.id) {
        if (Array.isArray(event.unreadStoryIds)) {
          this.unreadDiscussionStoryIds = new Set(event.unreadStoryIds);
        }
      } else if (!isOpenedDiscussion) {
        this.unreadDiscussionStoryIds.add(event.storyId);
        this.cdr.markForCheck();
      }

      if (isOpenedDiscussion) {
    ...
    if (event.eventType === 'READ_STATE_UPDATED' && event.actorUserId === this.currentUser.id) {
      if (Array.isArray(event.unreadStoryIds)) {
        this.unreadDiscussionStoryIds = new Set(event.unreadStoryIds);
        this.cdr.markForCheck();
        return;
  ...
  private handleTaskDiscussionRealtimeEvent(event: TaskDiscussionRealtimeEvent): void {
    if (this.currentUser == null || this.selectedProjectId == null || event.projectId !== this.selectedProjectId) {
      return;
    }

    if (event.eventType === 'MESSAGE_CREATED') {
      const openedTaskId = this.taskDiscussionDialogOpen ? this.discussionTask?.id : null;
      const isOpenedDiscussion = openedTaskId != null && openedTaskId === event.taskId;

      if (event.actorUserId === this.currentUser.id) {
        if (Array.isArray(event.unreadTaskIds)) {
          this.unreadDiscussionTaskIds = new Set(event.unreadTaskIds);
        }
      } else if (!isOpenedDiscussion) {
        this.unreadDiscussionTaskIds.add(event.taskId);
        this.cdr.markForCheck();
      }

      if (isOpenedDiscussion) {
        this.loadTaskDiscussionMessages(false);
        if (event.actorUserId !== this.currentUser.id) {
          this.markOpenedTaskDiscussionAsRead();
        }
      }
  ...
  private refreshUnreadStoryDiscussionIds(): void {
    ...
    this.taskService.getUnreadStoryDiscussionIds(this.selectedProjectId, this.currentUser.id)
      .subscribe({
        ...
        error: () => {
          // Do not block the rest of the screen when unread polling fails.
        }
      });
  }

  private refreshUnreadTaskDiscussionIds(): void {
    if (this.currentUser == null || this.selectedProjectId == null) {
      this.unreadDiscussionTaskIds.clear();
      this.cdr.markForCheck();
      return;
    }

    this.taskService.getUnreadTaskDiscussionIds(this.selectedProjectId, this.currentUser.id)
      .subscribe({
        next: taskIds => {
          this.unreadDiscussionTaskIds = new Set(taskIds);
          this.cdr.markForCheck();
        },
        error: () => {
          // Do not block the rest of the screen when unread polling fails.
  ...
  private resetTaskForm(): void {
    this.editingTaskId = null;
    this.taskForm.storyId = this.selectedStoryId ?? this.taskForm.storyId;
    this.taskForm.title = '';
  ...
  private scrollTaskFormIntoView(): void {
    requestAnimationFrame(() => {
      this.taskFormCardRef?.nativeElement.scrollIntoView({
        behavior: 'smooth',
        block: 'start'
      });
    });
    ...
    this.taskService.getProjects().subscribe({
      next: projects => {
        ...
        if (selectedProject == null) {
          this.errorMessage = 'Vybraný projekt neexistuje nebo do něj nemáš přístup.';
          this.selectedProjectName = '';
          this.resetProjectContext();
          return;
        }
        ...
        this.errorMessage = '';
        this.loadUsers(selectedProject.id);
        this.subscribeToDiscussionRealtime();
        this.subscribeToTaskDiscussionRealtime();
        this.refreshUnreadStoryDiscussionIds();
        this.refreshUnreadTaskDiscussionIds();
