...
export default function OverdraftPage() {
  ...
  return <BankShell><div className="bank-content section-page loans-page">
    ...
    <section className="loan-applications"><div className="section-heading"><div><h2>Výpis kontokorentu</h2><p>Stav vašich žádostí a rozhodnutí banky.</p></div><span>{applications.length}</span></div>{applications.length === 0 ? <p className="bank-empty">Zatím nemáte žádnou žádost o kontokorent.</p> : applications.map((item) => <article key={item.id}><span className={`loan-status ${item.status === "REJECTED" ? "loan-status-rejected" : ""}`}><CheckCircle2 size={18} /></span><div><strong>Kontokorent</strong><small>Odesláno {new Intl.DateTimeFormat("cs-CZ", { dateStyle: "medium" }).format(new Date(item.createdAt))}</small></div><div><strong>{currency.format(item.amount)}</strong><small>{currency.format(item.monthlyIncome)} měsíční příjem</small></div><b className={`loan-application-status status-${item.status.toLowerCase()}`}>{applicationStatusLabel(item.status)}</b></article>)}</section>
