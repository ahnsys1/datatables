...
export default function LoansPage() {
  ...
  function openRepayment(application: LoanApplication, repayAll: boolean) {
    ...
    setRepaymentAmount(repayAll ? application.earlyRepaymentAmount ?? application.remainingAmount ?? Math.round(application.monthlyPayment * remainingPayments * 100) / 100 : application.monthlyPayment);
    setError("");
  }
  ...
  return <BankShell><div className="bank-content section-page loans-page">
    ...
    {applications.length > 0 && <section className="loan-applications"><div className="section-heading"><h2>Moje půjčky a žádosti</h2><span>{applications.length}</span></div><div className="loan-table-wrap"><table className="loan-table"><thead><tr><th>Produkt</th><th>Částka a sazba</th><th>Stav splácení</th><th>Termín a symboly</th><th>Stav</th><th>Akce</th></tr></thead><tbody>{applications.map((application) => { const approved = application.status === "APPROVED"; const canRepay = approved && (application.remainingAmount ?? 0) > 0; const rejected = application.status === "REJECTED"; return <tr key={application.id}><td><strong>{products[application.type === "PERSONAL" ? "personal" : "home"].name}</strong><small>Odesláno {new Intl.DateTimeFormat("cs-CZ", { dateStyle: "medium" }).format(new Date(application.createdAt))}</small></td><td><strong>{currency.format(application.amount)}</strong><small>{application.annualRate.toLocaleString("cs-CZ")} % p. a. · {currency.format(application.monthlyPayment)} měsíčně</small></td><td>{approved ? <><strong>Zbývá {currency.format(application.remainingAmount ?? 0)}</strong><small>Uhrazeno {currency.format(application.repaidAmount ?? 0)} · zbývá {application.remainingInstallments ?? application.repaymentMonths} splátek</small></> : <small>{rejected ? "Žádost o půjčku byla zamítnuta." : "Údaje budou dostupné po schválení."}</small>}</td><td>{approved ? <><strong>{application.dueDate ? new Intl.DateTimeFormat("cs-CZ").format(new Date(application.dueDate)) : `${application.repaymentDayOfMonth}. den v měsíci`}</strong><small>{application.repaymentAccountNumber}<br />VS {application.variableSymbol} · SS {application.specificSymbol}</small></> : <small>-</small>}</td><td><span className={`loan-status-label ${application.status.toLowerCase()}`}>{application.status === "PENDING" ? "Čeká na posouzení" : application.status === "APPROVED" ? "Schváleno" : "Zamítnuto"}</span></td><td>{canRepay ? <span className="loan-repayment-actions"><button type="button" onClick={() => openRepayment(application, false)}>Splátka</button><button type="button" onClick={() => openRepayment(application, true)}>Doplatit</button></span> : <small>-</small>}</td></tr>; })}</tbody></table></div></section>}
