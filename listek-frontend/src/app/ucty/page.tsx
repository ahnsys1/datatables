...
export default function AccountsPage() {
  ...
  return <BankShell><div className="bank-content section-page">
    ...
    <div className="account-page-grid">
      ...
      {savingsAccount && <article className="savings-account account-detail-card"><div className="savings-head"><span><TrendingUp size={19} /> Spořicí účet</span><Link className="row-arrow" href={`/payments?accountId=${savingsAccount.id}`} aria-label="Zobrazit pohyby spořicího účtu"><ArrowRight size={17} /></Link></div><p className="account-number">{savingsAccount.accountNumber}</p><strong>{visible ? `${savingsAccount.balance.toLocaleString("cs-CZ", { minimumFractionDigits: 2 })} ${savingsAccount.currency}` : "••••••••"}</strong><p>Účet je veden v měně {savingsAccount.currency}.</p><div className="saving-progress"><span style={{ width: "0%" }} /></div><small>Spořicí cíl zatím není nastaven.</small></article>}
    </div>
