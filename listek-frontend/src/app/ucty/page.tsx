...
export default function AccountsPage() {
  ...
  const savingsAccount = accounts.find((account) => account.id !== primaryAccount?.id);
  return <BankShell><div className="bank-content section-page">
    ...
    <div className="account-page-grid">
      <article className="account-primary account-detail-card"><div className="account-topline"><span className="account-type">Běžný účet</span><Landmark size={20} />{primaryAccount && <Link className="row-arrow" href={`/payments?accountId=${primaryAccount.id}`} aria-label="Zobrazit pohyby běžného účtu"><ArrowRight size={17} /></Link>}</div><p className="account-number">{primaryAccount?.accountNumber ?? "-"}</p><p className="balance-label">Disponibilní zůstatek</p><strong className="main-balance">{visible ? primaryAccount ? `${primaryAccount.balance.toLocaleString("cs-CZ", { minimumFractionDigits: 2 })} ${primaryAccount.currency}` : "-" : "••••••••"}</strong><div className="account-footer"><span><i /> Aktivní účet</span><button onClick={() => primaryAccount && setDetailAccount(primaryAccount)}>Detail účtu <ArrowRight size={15} /></button></div></article>
      <article className="savings-account account-detail-card"><div className="savings-head"><span><TrendingUp size={19} /> Spořicí účet</span>{savingsAccount && <Link className="row-arrow" href={`/payments?accountId=${savingsAccount.id}`} aria-label="Zobrazit pohyby spořicího účtu"><ArrowRight size={17} /></Link>}</div>{savingsAccount ? <><p className="account-number">{savingsAccount.accountNumber}</p><strong>{visible ? `${savingsAccount.balance.toLocaleString("cs-CZ", { minimumFractionDigits: 2 })} ${savingsAccount.currency}` : "••••••••"}</strong><p>Účet je veden v měně {savingsAccount.currency}.</p><div className="saving-progress"><span style={{ width: "0%" }} /></div><small>Spořicí cíl zatím není nastaven.</small></> : <><strong>Účet není dostupný</strong><p>Nemáte založený druhý účet pro spoření.</p></>}</article>
    </div>
