...
import { Account, InterestSettings, createSavingsAccount, createStandingOrder, getAccounts, getInterestSettings, transferMoney } from "../../lib/api";
...
export default function SavingsPage() {
  ...
  useEffect(() => {
    Promise.all([getAccounts(), getInterestSettings()]).then(([loadedAccounts, loadedRates]) => { setAccounts(loadedAccounts); setRates(loadedRates); }).catch(() => setMessage("Účty se nepodařilo načíst."));
  }, []);
  ...
  return <BankShell><div className="bank-content section-page">
    ...
    <div className="savings-hero"><div className="savings-copy"><span className="savings-icon"><TrendingUp size={23} /></span><p className="modal-kicker">Spořicí účet</p>{savingsAccount ? <><p className="account-number">Číslo účtu: {savingsAccount.accountNumber}</p><h2>{savingsAccount.balance.toLocaleString("cs-CZ", { style: "currency", currency: savingsAccount.currency })}</h2><p>Úroková sazba <b>{(rates?.savingsRate ?? 4.2).toLocaleString("cs-CZ")} % p. a.</b></p><p className="account-status"><i /> Aktivní účet</p><div className="savings-actions"><button className="text-button" onClick={() => setModal("transfer")}>Převést peníze <ArrowRight size={16} /></button><button className="text-button" onClick={() => setModal("order")}>Nastavit pravidelné spoření <Plus size={16} /></button></div></> : <><h2>Spořicí účet zatím nemáte</h2><p>Založte si ho a začněte si odkládat peníze.</p><button className="pay-button" onClick={() => void openSavingsAccount()} disabled={saving || !sourceAccount}>{saving ? "Zakládám..." : "Založit spořicí účet"}</button></>}</div>{savingsAccount && <div className={`goal-ring${primaryGoal ? "" : " goal-ring-empty"}`}><strong>{progress}%</strong><span>{primaryGoal?.name ?? "bez cíle"}</span></div>}</div>
