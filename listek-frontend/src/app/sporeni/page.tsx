...
import { Account, createStandingOrder, getAccounts, transferMoney } from "../../lib/api";
...
export default function SavingsPage() {
  ...
  return <BankShell><div className="bank-content section-page">
    ...
    <div className="savings-hero"><div className="savings-copy"><span className="savings-icon"><TrendingUp size={23} /></span><p className="modal-kicker">Spořicí účet</p><h2>{(savingsAccount?.balance ?? 0).toLocaleString("cs-CZ", { style: "currency", currency: savingsAccount?.currency ?? "CZK" })}</h2><p>Úroková sazba <b>4,2 % p. a.</b></p><div><button className="text-button" onClick={() => setModal("transfer")}>Převést peníze <ArrowRight size={16} /></button><button className="text-button" onClick={() => setModal("order")} disabled={!savingsAccount}>Nastavit pravidelné spoření <Plus size={16} /></button></div></div><div className="goal-ring"><strong>{progress}%</strong><span>{primaryGoal?.name ?? "bez cíle"}</span></div></div>
